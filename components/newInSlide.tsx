...
const NewInSlide = () => {
    ...
    const addNewIn = (value: string) => {
        if (!newIn.includes(value)) {
            setNewIn([...newIn, value]);
        }
    };

    const removeNewIn = (value: string) => {
        setNewIn(newIn.filter(seller => seller !== value));
    };
                ...
                <ul className="space-y-2 lg:w-[28%]">
                    {['Taskane Marina', 'Oud Zafran', 'Supremacy Noir'].map((item) => (
                        <li key={item} className="flex justify-between items-center">
                            <span>{item}</span>
                            <div>
                                <Button size="sm" variant="outline" className='hover:bg-black hover:text-white rounded-none' onClick={() => addNewIn(item)}>+</Button>
                                <Button size="sm" variant="outline" onClick={() => removeNewIn(item)} className="ml-5 hover:bg-black hover:text-white rounded-none">-</Button>
                            </div>
                        </li>
                    ))}
                </ul>
