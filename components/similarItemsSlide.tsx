...
const SimilarItemsSlide = () => {
    const [products, setProducts] = useState<Product[]>(initialProducts);
    const [similarItems, setSimilarItems] = useState<string[]>([]);

    const addSimilarItems = (value: string) => {
        if (!similarItems.includes(value)) {
            setSimilarItems([...similarItems, value]);
        }
    };

    const removeSimilarItems = (value: string) => {
        setSimilarItems(similarItems.filter(seller => seller !== value));
    };
    ...
    return (
        <>
            ...
            <div>
                <h2 className="text-base font-semibold text-center md:text-left bg-gray-50 py-1 pl-2 mb-6">Add Similar Items</h2>
                ...
                <ul className="space-y-2 lg:w-[28%]">
                    {['Taskane Marina', 'Oud Zafran', 'Supremacy Noir'].map((item) => (
                        <li key={item} className="flex justify-between items-center">
                            <span>{item}</span>
                            <div>
                                <Button size="sm" variant="outline" className='hover:bg-black hover:text-white rounded-none' onClick={() => addSimilarItems(item)}>+</Button>
                                <Button size="sm" variant="outline" onClick={() => removeSimilarItems(item)} className="ml-5 hover:bg-black hover:text-white rounded-none">-</Button>
                            </div>
                        </li>
                    ))}
                </ul>
...
export default SimilarItemsSlide;
